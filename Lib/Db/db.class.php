 ...
 * @version  $Id: db.class.php 19 2011-03-16 06:54:28Z pengzl_gz@163.com $
  +------------------------------------------------------------------------------
 */
class db {
    ...
    public function __construct($conf = null) {
        if (empty(self::$configs) || !empty($conf)) {
            ...
            if (is_array($conf)) {
                ...
                self::$configs['dbhost'] = C('DB_HOST');
                self::$configs['dbuser'] = C('DB_USER');
                self::$configs['dbpass'] = C('DB_PWD');
                self::$configs['dbname'] = C('DB_NAME');
                self::$configs['dbport'] = C('DB_PORT');
                self::$configs['charset'] = C('DB_CHARSET');
            }
        }

        is_object(self::$DB) || self::connect();
        empty(self::$configs['charset']) && self::$configs['charset'] = 'utf8';
        empty(self::$debug) && self::$debug = C('DEBUG_MODE');
    ...
    static private function connect() {
        if (!isset(self::$DB) || !is_object(self::$DB)) {
            ...
            if ($mysqli->connect_errno || $mysqli->connect_errno === NULL) {
                throw new Exception(L('_DB_CONNECT_ERROR_') . $mysqli->connect_error);
            }
    ...
    public function _query($sql) {
        if (self::$debug) {
            PLFException::$_SqlStr[] = "{$sql}<br />";
        }

        if (false === $result = self::$DB->query($sql)) {
            throw new Exception(L('_SQL_SELECT_ERROR_') . self::$DB->error . ' , SQL: ' . $sql);
        }
    ...
    public function insert($table, $avgs, $into = false) {
        return self::runUpdateSQL(boolVal($into) ? 'insert_into' : 'insert', $table, $avgs);
    }
    ...
    public function execute($sql) {
        if ($sql == false || empty($sql)) {
            throw new Exception(L('_SQL_ISEMPTY_ERROR_'));
            return false;
        }
    ...
    private function whereKV($where) {
        ...
        return msubstr(trim($_res), mb_strlen($_operate, 'utf8') - 2);
    }
     ...
     * @desc 写操作跟踪日志
     */
    private static function doTrackLog($sql = '') {
        if (C('TRACK_LOG') && in_array($str = strtolower(substr($sql, 0, 6)), array('update', 'delete', 'insert'))) {
            $_escp = array('update' => '更新', 'delete' => '删除', 'insert' => '添加');
            $sql = strtolower($sql);

            $tables = explode($str, $sql);
            $tables = explode('set', $tables[1]);
            $tables = $tables[0];

            $conts = '[' . S(C('USER_SESSION_KEY')) . '] in ' . date('Y-m-d H:i:s') . ' ' . $_escp[$str] . ':[' . $tables . '] sql:[' . urlencode($sql) . ']';
            return error_log("$conts\n", FILE_LOG, APP_PATH . '/logs/sys_action_' . date('Y-m-d H') . '.log');
            //return filePutTxt(APP_PATH . '/logs/sys_action_' . date('Y-m-d') . '.log', $conts);
        }
        return false;
    }
