...
1> 框架特点简洁干净，适用于对大中小型网站的统一开发与管理，尤其适用于对生成纯静态页面的
    中小型网站的开发。框架主程序简洁轻便，路径解析简单，框架模板为SMARTY，并集各常用函数、分页、
    图片上传综合处理、数据库表数据导入导出、静态页面生成、基于MemCache的缓存类、基于phpmailer的邮件收发类等类于一体，方便扩展。

2> 框架环境配置，共分成三类分三个文件存（系统定义文件defines.php、系统配置文件convention.php、环境配置文件environment.php），
    其中系统配置文件分为两份，一份为系统默认的配置文件存于/PLFrame/Common/convention.php，
    另一份为自定义项目配置文件存放于/项目名/conf/config.inc.php，如果项目配置文件为空或不存在则不加载项目配置
    文档，一般不建议更改框架的系统配置文件convention.php，若有需要则可更改增减自定义的项目配置文件。同样系统
    定义文件也分为两份，一份为系统默认的定义文件存放于/PLFrame/Common/defines.php此系统定义文件若非绝对必要则请勿更改，
    否则有可能造成系统不能运作。另一份为具体项目所用到的各类区别于系统定义的参数定义，存放于/项目名/conf/defines.php，
    有需要可自行创建此文件存放于此即可自行加载，同样如果项目定义文件为空或不存在则不加载.系统环境配置文件存放于/PLFrame/Common/environment.php,
    一般不需做更改。框架是通过统一的项目入口文件index.php来完成项目所要求的环境配置与各类及逻辑文件的加载的。
...
4> 框架是基于伪MVC三层结构方式实现，M层暂时划分成两块，一块是数据库中间处理，另一块是逻辑层实现功能
    所需的数据处理部分；其实现也分成两部分，第一部分是由框架主程序下的一个DB类来封装逻辑层与数据库的交换数据
    处理工作，另一部分就是逻辑层本身为现实业务层功能所做的数据处理都封装于与本逻辑实现程序同名的类里面，其默
    认存放目录为/ui/module/module名/action名.class.php；V层则通过SMARTY类来解析实现；C层则是通过简单的路径解析
    来定位指定逻辑控制文件的方式实现。简单路径解析说明下如：
       框架默认采用传统URL参数形式，路由主要通过action与module这两个特定参数来实现，两参数名字可在/PLFrame/Common/defines.php
    里面定义，也可以在项目目录/conf/新建defines.php来定义,如:
    define('URL_MODULE', 'MOD');    // URL模块名参数
    define('URL_ACTION', 'ACT');   // URL文件名参数
    
    action表示逻辑文件名（不带后缀），module表示逻辑文件所属目录。具体如：http://www.plframe.com/?ACT=login&MOD=user
    解析后实际指向的文件为 /项目逻辑文件路径/module名/action名.php。若设项目逻辑文件路径=ui/pages，则上述地址通过解析后实际加载
    的文件为 /ui/pages/user/login.php，若在地址栏非通过项目入口文件index.php而直接加载项目目录下某个文件，则此文件将处于脱离框架环境独
    立运行状态。
...
1> Db类库调用方法及说明
    ...
    $_array_Insert = array('value'=>array('a_name'=>'文章名', 'a_title'=>'文章标题', 'a_content'=>'文章内容'));
    ...
    $sql = $_DBObj->formatSQL('select', 'fyh_article', $_array_select); // 返回拼装后的SQL语句
    //返回: 'SELECT `id`, `a_name` FROM `fyh_article` WHERE `id` = 1 AND `fid` = 2 ORDER BY id desc LIMIT 60'.
...
4> 公用图片处理类说明
    ...
    $saveFileName 	= time() . mt_rand(0, 999999) . "." . $filename_info['extension']; // 生成上传文件保存名字。
